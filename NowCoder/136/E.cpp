...
int s[N][N];
int n;
char name[][10] = {"HtBest", "WHZ"};
inline int gao(int x, int y, int i, int j, int d)
{
    for (int t = 0; t < 5; t++)
    {
        int tx = x + i * t, ty = y + j * t;
        if (tx < 1 || ty < 1 || tx > n || ty > n) return t - 1;
        if (s[tx][ty] != d) return t - 1;
    }
    return 4;
}
inline void judge(int x, int y, int d, int m)
{
    if (gao(x, y, -1, -1, d) + gao(x, y, 1, 1, d) >= 4)
    {
        io << name[d] << " " << m << nl;
        exit(0);
    }
    if (gao(x, y, -1, 0, d) + gao(x, y, 1, 0, d) >= 4)
    {
        io << name[d] << " " << m << nl;
        exit(0);
    }
    if (gao(x, y, 0, -1, d) + gao(x, y, 0, 1, d) >= 4)
    {
        io << name[d] << " " << m << nl;
        exit(0);
    }
