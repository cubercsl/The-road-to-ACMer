...
{
    for (; x; x -= x & -x) (bit[x] += y) %= mod;
}

ll query(int x)
{
    ll ans = 0;
    for (; x < N; x += x & -x) (ans += bit[x]) %= mod;
    return ans;
}
...
void go()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        (ans += query(x) * (n - i) % mod) %= mod;
        update(x, 1LL * (i + 1) * a[i] % mod);
    }
    memset(bit, 0, sizeof(bit));
    for (int i = n - 1; ~i; i--)
    {
        int x = lower_bound(b.begin(), b.end(), a[i]) - b.begin() + 1;
        (ans += query(x) * (i + 1) % mod) %= mod;
        update(x, 1LL * (n - i) * a[i] % mod);
    }
    for (int i = 0; i < n; i++) (ans += 1LL * (i + 1) * (n - i) % mod * a[i] % mod) %= mod;
    W(ans);
}
