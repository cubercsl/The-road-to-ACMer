...
void go()
{
    ...
    for (int i = 0, x; i < n; i++)
    {
        ...
        if (op == "ADD")
        {
            int lb = b.size() ? *b.begin() : INF;
            int ub = c.size() ? *c.rbegin() : -INF;
            (x > lb ? b : (x < ub ? c : a)).insert(x);
        }
        else
        {
            int lb = b.size() ? *b.begin() : INF;
            ...
            if (a.count(x))
                (res <<= 1) %= mod, a.erase(x);
            if ((b.count(x) && lb != x) || (c.count(x) && ub != x))
                return (void)W(0);
            if (lb == x) b.erase(x);
            if (ub == x) c.erase(x);
            for (auto t : a) (t < x ? c : b).insert(t);
            a.clear();
        }
    }
    res = res * (static_cast<ll>(a.size()) + 1) % mod;
    W(res);
}
