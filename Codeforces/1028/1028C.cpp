...
struct rectangle
{
    int d, l, u, r;
    rectangle() {}
    rectangle(int d, int l, int u, int r) : d(d), l(l), u(u), r(r) {}
    rectangle operator|(const rectangle& rhs)
    {
        return {max(d, rhs.d), max(l, rhs.l), min(u, rhs.u), min(r, rhs.r)};
    }
} r[N], pre[N], suf[N];
...
void go()
{
    int n;
    R(n);
    for (int i = 1; i <= n; i++) R(r[i].d, r[i].l, r[i].u, r[i].r);
    pre[0] = suf[n + 1] = {-INF, -INF, INF, INF};
    for (int i = 1; i <= n; i++) pre[i] = pre[i - 1] | r[i];
    for (int i = n; i; i--) suf[i] = suf[i + 1] | r[i];
    for (int i = 1; i <= n; i++)
    {
        rectangle now = pre[i - 1] | suf[i + 1];
        if (now.d <= now.u && now.l <= now.r)
            return (void)W(now.d, now.l);
    }
}
