...
ll gao(int n, int m, int i, int j)
{
    ll tn = n, tm = m, ti = i, tj = j;
    return ti * (ti - 1) * tm / 2
           + (tn - ti) * (tn - ti + 1) * tm / 2
           + tj * (tj - 1) * tn / 2
           + (tm - tj) * (tm - tj + 1) * tn / 2;
...
bool check(int n, int m, int x, int y, const VI& a)
{
    VI b;
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++)
            b.push_back(abs(i - x) + abs(j - y));
    sort(b.begin(), b.end());
    for (int i = 0; i < n * m; i++)
...
void go()
{
    ...
    ll ans = 0;
    VI a(t);
    for (int i = 0; i < t; i++) R(a[i]), ans += a[i];
    sort(a.begin(), a.end());
    for (int i = 1; i * i <= t; i++)
    {
        if (t % i) continue;
        ...
        for (int j = 1; j <= u; j++)
        {
            ...
            {
                int mid = l + r >> 1;
                ll tmp = gao(n, m, j, mid);
                if (tmp == ans)
                {
                    if (check(n, m, j, mid, a))
                    {
                        W(n, m);
                        W(j, mid);
                        return;
