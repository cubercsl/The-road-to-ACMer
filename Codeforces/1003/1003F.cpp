...
void go()
{
    ...
    auto get_id = [&](const string& s) { return id[s] ? id[s] : id[s] = id.size(); };
    VI a(n), len(n);
    for (int i = 0; i < n; i++)
    {
        static string s;
    ...
    int sum = accumulate(len.begin(), len.end(), n - 1);
    int ans = sum;

    auto kmp = [&](const VI& x, const VI& y) {
