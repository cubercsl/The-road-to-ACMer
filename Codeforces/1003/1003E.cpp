...
void go()
{
    ...
    vector<PII> ans;
    int p = 0;
    for (int i = 0; i < d; i++) ans.emplace_back(i + 1, i + 2), ++p;
    --k;
    function<void(int, int, int)> dfs = [&](int u, int dep, int b) {
        if (dep == b)
            return;
        if (!dep)
        {
            for (int i = 0; i < k - 1 && p != n - 1; ++i)
            {
                ans.emplace_back(u, p + 2);
                dfs((++p + 1), dep + 1, b);
            }
        }
        else
        {
            for (int i = 0; i < k && p != n - 1; ++i)
            {
                ans.emplace_back(u, p + 2);
                dfs((++p + 1), dep + 1, b);
            }
