...
public:
    Base() { r = 0, a.fill(0); }
    Base(const Base& rhs) { (*this) = rhs; }
    void clear() { r = 0, a.fill(0); }
    bool insert(T x)
    {
        for (int i = D - 1; ~i; i--)
            if (x >> i & 1)
...
long long p[1 << 17];
long long a[1 << 17];
Base<long long, 63> pre, suf[1 << 17], s;
bool ok[1 << 17];
const int mod = 1e9 + 7;
...
{
    p[0] = 1;
    for (int i = 1; i < (1 << 17); i++) p[i] = (p[i - 1] << 1) % mod;
    int n;
    while (io >> n)
    {
        s.clear();
        ...
        for (int i = n - 1; ~i; i--) suf[i] = suf[i + 1], suf[i].insert(a[i]);
        for (int i = 0; i < n; i++) ok[i] = s.insert(a[i]);
        long long ans = 0;
        for (int i = 0; i < n; i++)
        {
            if (!ok[i])
                add(ans, p[n - 1 - s.rank()]);
            else
            {
                Base<long long, 63> t(pre);
                t += suf[i + 1];
                if (!t.insert(a[i])) add(ans, p[n - 1 - t.rank()]);
            }
            pre.insert(a[i]);
        }
        io << ans << nl;
