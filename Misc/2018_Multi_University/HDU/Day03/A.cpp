...
int main()
{
    ...
    while (T--)
    {
        ...
        ans = 0, cnt = 0;
        io >> n >> m >> k >> p >> q >> r >> mod;
        for (i = 1; i <= k; i++) io >> a[i];
        for (i = k + 1; i <= n; i++) a[i] = (1LL * p * a[i - 1] + 1LL * q * i + r) % mod;
        int head = 0, tail = 0;
        for (i = 1; i <= n; i++)
        {
            while (head != tail && a[i] > a[dq[tail - 1]]) R[dq[tail - 1]] = i, --tail;
            dq[tail++] = i;
        }
        while (head != tail) R[dq[tail - 1]] = n + 1, --tail;
        dp[n] = 1;
        head = tail = 0;
        for (i = n - 1; i; i--) dp[i] = dp[R[i]] + 1;
        for (i = 1, j = 1; i + m - 1 <= n; i++)
        {
            while (j < i + m)
            {
            ...
            ans = ans + (a[dq[head]] ^ i);
            cnt = cnt + ((dp[i] - dp[dq[head]] + (a[i] != 0)) ^ i);
        }
        io << ans << " " << cnt << nl;
    }
}
