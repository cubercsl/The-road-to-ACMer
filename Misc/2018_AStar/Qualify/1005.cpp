...
struct BIT
{
    int n;
    int t[N];
    void init(int n) { this->n = n, memset(t, 0, sizeof(int) * n); }
...
int main()
{
    ...
    {
        int n = io.ri();
        for (int i = 0; i <= 200; i++) t[i].init(n + 1);
        for (int i = 1; i <= n; i++) a[i] = io.ri();
        for (int i = 1; i <= n; i++) dp[i] = 0;
        for (int i = 1; i <= n; i++)
        {
            for (int j = min(200, i); j; j--)
            {
                int tmp = j == 1 ? 1 : t[j - 1].query(a[i] - 1);
                // cerr << "[" << i << "," << j << " ]" << tmp << endl;
                dp[j] += tmp;
