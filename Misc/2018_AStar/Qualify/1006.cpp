...
struct Edge
{
    ...
    int color;
    Edge() {}
    Edge(int u, int v, int w, int c) : from(u), to(v), cost(w), color(c) {}
...
int main()
{
    ...
    while (T--)
    {
        ...
        for (auto& e : edges)
        {
            if (e.color == 0) a2[p2++] = e.cost;
            if (e.color == 2) a1[p1++] = e.cost;
            if (e.color == 0 || e.color == 1)
            {
                if (d1.same(e.from, e.to))
                    a1[p1++] = e.cost;
                else
                    d1.unite(e.from, e.to), a1[n - 1] += e.cost, ++c1;
            }
            if (e.color == 1 || e.color == 2)
            {
                if (d2.same(e.from, e.to))
                    a2[p2++] = e.cost;
                else
                    d2.unite(e.from, e.to), a2[n - 1] += e.cost, ++c2;
            }
        }
        if (c1 == n - 1)
            for (int i = n; i < p1; i++) a1[i] += a1[i - 1];
        else
            for (int i = n - 1; i < p1; i++) a1[i] = INF;

        if (c2 == n - 1)
            for (int i = n; i < p2; i++) a2[i] += a2[i - 1];
        else
            for (int i = n - 1; i < p2; i++) a2[i] = INF;

        io << "Case #" << ++kase << ":" << nl;
        for (int i = 1; i < n - 1; i++) io << -1 << nl;
        for (int i = n - 1; i <= m; i++)
        {
            int ans = min(a1[i], a2[i]);
            if (ans == INF)
                io << -1 << nl;
            else
                io << ans << nl;
