...
ll inv2;
inline ll gao(int x, int y) { return (1LL * (x + y) * (y - x + 1) % mod) * inv2 % mod; }
int main()
{
    inv2 = Pow(2);
    int T = io.ri();
    while (T--)
    {
        ...
        int xx = 0, yy = 0;
        for (int i = 0; i < n; i++) io >> a[i].first >> a[i].second, xx = max(xx, a[i].first), yy = max(yy, a[i].second);
        ll ans = 0;
        for (int i = xx; i <= yy; i++)
        {
            ll tmp = 1;
            ...
            for (int j = n - 1; ~j; j--) (w[j] *= w[j + 1]) %= mod;
            for (int j = 0; j < n; j++)
            {
                if (a[j].second >= i) ans = (ans + tmp * w[j + 1]) % mod;
        ...
        ll tmp = 1;
        for (auto& t : a) tmp = tmp * (t.second - t.first + 1) % mod;
        ans = ans * Pow(tmp) % mod;
        io << ans << nl;
    }
}
