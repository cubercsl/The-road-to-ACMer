...
int main()
{
    ...
    while (T--)
    {
        ...
        for (int i = 1; i <= n; i++) io >> val[i], dp[i][0] = dp[i][1] = dp[i][2] = dp[i][3] = 0;
        dp[0][0] = dp[0][1] = dp[0][2] = dp[0][3] = 0;
        for (int i = 1; i <= n; i++)
        {
            if (val[i] >= dp[fa[i]][0])
                dp[fa[i]][1] = dp[fa[i]][0], dp[fa[i]][0] = val[i];
            else
                dp[fa[i]][1] = max(dp[fa[i]][1], val[i]);
            if (val[i] <= dp[fa[i]][2])
                dp[fa[i]][3] = dp[fa[i]][2], dp[fa[i]][2] = val[i];
            else
                dp[fa[i]][3] = min(dp[fa[i]][3], val[i]);
        }
        ll ans1 = 0, ans2 = 0;
        ll maxv = 0, minv = 0;
        for (int i = 0; i <= n; i++)
